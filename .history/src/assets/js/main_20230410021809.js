...
function saveStyles(elements) {
  ...
  mm.addEventListener("refreshInit", () => {
    matchedMedia = false;
    styles = elements.map(el => el.style.cssText);
  });
  mm.addEventListener("matchMedia", () => {
    matchedMedia = true;
  })
  mm.addEventListener("refresh", () => matchedMedia || elements.forEach((el, i) => el.style.cssText = styles[i]));
}
...
mm.add({
  ...
  function logoEvent() {
    ...
    // console.log(subSmallEl.clientHeight)
