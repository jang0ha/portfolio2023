...
mm.add("(min-width: 992px)", (context) => {
  ...
  window.addEventListener("mouseenter", context.linkHover);


  return () => { // make sure to clean up event listeners in the cleanup function!
    window.removeEventListener("mouseenter", context.linkHover);
  }
  
});

const projectItemEl = document.querySelector.all(".project-item");
