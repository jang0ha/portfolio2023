...
mm.add("(min-width: 992px)", (context) => {
  ...
  context.add("linkHover",linkHover);
  context.add("linkHover",moveImg);



  projectItemEl.forEach((item) => {
    item.addEventListener("mouseenter", context.linkHover);
    item.addEventListener("mouseleave", context.linkHover);
    item.addEventListener("mousemove", context.moveImg);
  })

  return () => { // make sure to clean up event listeners in the cleanup function!
    projectItemEl.forEach((item) => {
      item.removeEventListener("mouseenter", context.linkHover);
      item.removeEventListener("mouseleave", context.linkHover);
      item.removeEventListener("mousemove", context.moveImg);
    })
  }
...
const projectItemEl = gsap.utils.toArray(".project-item");
let projectThumbImage = projectItemEl.querySelector('.thumb-img');
...
function move() {
  projectItemEl.forEach((item) => {
    item.addEventListener("mouseenter",linkHover);
    item.addEventListener("mouseleave", linkHover);
    item.addEventListener("mousemove", moveImg);
  })
}