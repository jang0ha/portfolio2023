...
const projectSwiperEl = gsa.utils.toArray(".project-list"); 
const projectLinkEl = document.querySelector(".project-link")];
const projectLinkThumbEl = projectLinkEl.querySelector(".thumb-img");

function moveNextImage(e) {
  let xpos = e.offsetX;
  let ypos = e.offsetY;
...
function nextImageHover(e) {
  if (e.type === "mouseenter") {
    let xpos = e.offsetX;
    let ypos = e.offsetY;
    const t1 = gsap.timeline();
    t1.set(projectLinkThumbEl, {
      x: xpos,
      y: ypos
    }).to(projectLinkThumbEl, {
      duration: 0.5,
      autoAlpha: 1,
      scale:1
    });
