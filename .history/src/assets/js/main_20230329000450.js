...
document.addEventListener("DOMContentLoaded", function () {
  ...
  mm.add({
    ...
    context.add("headerEvent", () => {
      ...
      let headerCenter = (headerBlock.clientHeight - 32) / 2; //32px로 폰트 사이즈 강제로 줌,사파리로 인해 
    ...
    window.addEventListener("resize",  context.headerEvent)
     

    return () => {
      window.removeEventListener("resize",  context.headerEvent)
    }
  })
});
